...
- Axis는 워크스페이스 전체에서 **Supabase 사용이 승인된 유일한 프로젝트**다 (복잡한 RLS·관계형 구조 필요).
- 다른 프로젝트에 Supabase를 추가하지 않는다. Axis 안에서도 불필요한 유료 기능은 무료 티어로 유지.
- 신규 유료 서비스(AI 프로바이더, 가격 API 등) 도입 전 conductor 승인 필수.
- AI 프로바이더는 Groq 무료 티어(6,000 TPM)를 기본으로 사용하고, 트래픽 증가 시 폴백 체인 추가.
